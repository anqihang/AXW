...
const SOCKET_URL = apiConfig.SOCKET_URL; // 默认 WebSocket 地址
...
export function connectSocket(key: string, url: string = SOCKET_URL) {
  if (Object.keys(sockets).length === 5) return; // 限制最多5个连接
 ...
 * 监听消息
 */
export function onSocketMessage(key: string, callback: (msg: WechatMiniprogram.SocketTaskOnMessageListenerResult) => void) {
  const instance = sockets[key];
  if (instance && instance.socketTask) {
